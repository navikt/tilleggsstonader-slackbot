...
export const genererHtml = async (): Promise<String> => {
    ...
    const reposHtml = repos
        .sort(sort)
        .map(repo => repoHtml(repo))
    return `<html lang="no"><body>${reposHtml.join("\n")}</body></html>`;
}

const repoHtml = (repo: RepoStatus) => {
    const title = `<span style="font-size: ${headerSize(repo)}px"><a href="${repo.pullsUrl}">${repo.name}</a></span>`;
    const subtitle = `${repo.antallDependabot > 0 ? '<span style="font-size: 12px"> (Dependabots:' + repo.antallDependabot + ')</span>' : ''}`;
...
const listPrs = (repo: RepoStatus) => {
    return `<ul>
        ...
        return `<li>${prTitle} ${checkIcon} ${pr.author} (${antallDager(pr)} dager siden)</li>`;
    }).join("\n")}
    </ul>`;
};
...
const antallDager = (pr: PullRequest) =>
    Math.floor((new Date().getTime() - new Date(pr.createdAt.substring(0, 10)).getTime()) / DØGN_I_MS);
